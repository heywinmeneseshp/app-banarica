...
import { historialProgramacion, paginarHistorialProgramaciones } from '@services/api/programaciones';
...
const CAMPOS_A_MOSTRAR = [
  'fecha', 'contenedor', 'bl', 'semana', 'movimiento', 'conductor_id', 'vehiculo_id',
  'llegada_origen', 'salida_origen', 'llegada_patio', 'retiro_patio',
  'llegada_destino', 'cierre', 'salida_destino', 'estado_listado',
];

function formatFechaHora(iso) {
  if (!iso) return '';
  // "2026-08-14T09:32:10-05:00" -> "2026-08-14 09:32:10"
...
export default function ProgramadorHistorialModal({ show, onClose, programacionId }) {
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Filtros de la vista general (solo aplican cuando no hay un programacionId puntual)
  const [filtros, setFiltros] = useState({
    usuario: '', contenedor: '', bl: '', accion: '',
  ...
  const esGeneral = !programacionId;
  ...
  const cargar = async () => {
    ...
    try {
      if (programacionId) {
        const res = await historialProgramacion(programacionId);
        setItems(res?.data || []);
      } else {
        const res = await paginarHistorialProgramaciones(pagination, limit, filtros);
        setItems(res?.data || []);
  ...
  }, [show, programacionId, pagination]);
    ...
    <Modal show={show} onHide={onClose} size="lg" centered scrollable>
      <Modal.Header closeButton className="bg-dark text-white">
        <Modal.Title className="h6 mb-0">
          {esGeneral ? 'Historial general del Programador' : `Historial de la programación #${programacionId}`}
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {esGeneral && (
          <div className="row g-2 mb-3">
            ...
            <div className="col-6 col-md-3">
              <Form.Control
                ...
                value={filtros.bl}
                onChange={(e) => setFiltros((prev) => ({ ...prev, bl: e.target.value }))}
              />
            </div>
            <div className="col-6 col-md-3">
              <Form.Select
